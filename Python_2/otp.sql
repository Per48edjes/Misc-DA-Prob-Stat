WITH flxt_quotes_ff AS (with shipment_grain as
      (SELECT
       ...
       FROM
       analytics.legacy.supply_quotes
       GROUP BY 1),
      ...
      air_freight as (
        ...
        from analytics.legacy.bi_quote_service_items
        group by 1
      ),
    ...
    (SELECT
      ...
      from analytics.legacy.supply_quotes
      JOIN shipment_grain ON supply_quotes.shipment_id = shipment_grain.shipment_id
      left join analytics.core.quotes as quotes on quotes.shipment_id = supply_quotes.shipment_id
        and quotes.first_accepted_at is not null
        and quotes.voided_at is null
      left join analytics.core.quote_transit_times as quote_transit_times on quote_transit_times.quote_id = supply_quotes.quote_id
  ...
  ,  flxt_quoted_transit_times_ocean__edt AS (select
    ...
    from
      analytics.core.quote_transit_times as core_quote_transit_times
      left join analytics.core.quotes as core_quotes on core_quote_transit_times.quote_id = core_quotes.id
        and core_quotes.accepted_at is not null
        and core_quotes.voided_at is null

      left join analytics.core.data_science_enums as data_source ON core_quote_transit_times.data_source = data_source.enum_index
        and data_source.model_table = 'quote_transit_times'
        and data_source.enum_column = 'data_source'

      left join analytics.core.predicted_quote_transit_times as predicted on core_quote_transit_times.quote_id = predicted.quote_id
...
FROM analytics.legacy.prep_clients  AS flxt_prep_clients_shipments
LEFT JOIN analytics.legacy.prep_shipment_details  AS flxt_shipment_details ON (flxt_shipment_details."client_id") = flxt_prep_clients_shipments.client_id
          AND (flxt_shipment_details."is_live_shipment") = true -- this condition ensure cleans data

LEFT JOIN analytics.legacy.prep_shipment_container_movements  AS flxt_shipment_container_movements ON (flxt_shipment_details."shipment_id") = flxt_shipment_container_movements.shipment_id
LEFT JOIN analytics.legacy.bi_address_locations  AS destination ON flxt_shipment_container_movements.destination_address_id = (destination."address_id")
LEFT JOIN analytics.legacy.bi_air_shipments  AS flxt_air_shipments ON flxt_shipment_container_movements.shipment_id = flxt_air_shipments.shipment_id
LEFT JOIN analytics.legacy.prep_shipment_movements  AS flxt_shipments_otp ON (flxt_shipment_details."shipment_id") = flxt_shipments_otp.shipment_id
LEFT JOIN flxt_quotes_ff ON (flxt_shipment_details."shipment_id") = flxt_quotes_ff.shipment_id
LEFT JOIN analytics.legacy.supply_ocean_shipments  AS flxt_supply_ocean_shipments__final ON (flxt_shipment_details."shipment_id") = flxt_supply_ocean_shipments__final.shipment_id
